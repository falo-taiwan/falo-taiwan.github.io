# 銀河 ERP 內訓課程：Class 03 課程首頁 (index.md)

本目錄由歷史課程回顧與 7 大教學主軸構成，整合了 Class 03 課堂基礎教材與進階實戰手冊。

---

## 歷史課程回顧
* 🔗 **Class 01**：[Class 01 課程簡報 (🔒 密碼保護：attn)](https://drive.google.com/file/d/1lcTQQ4ceWf8-I-qxifTIbkTq8gwW0Pp7/view?usp=sharing)
* 🔗 **Class 02**：[Class 02 課程簡報 (🔒 密碼保護：attn)](https://drive.google.com/file/d/1FV_SEjwf68vgd_CH28ScJSjz1FiwxxTw/view?usp=sharing)
* 💡 *註：目前採用簡易密碼保護，未來將升級為基於 Google Apps Script (GAS) 驗證的高強度保密安全版本。*

---

## 實戰專案與進階手冊
* 🔗 **Antigravity**：[Antigravity 實戰開發與協作教學手冊](https://falo-taiwan.github.io/antigravity/)
* 🔗 **Chrome Ext**：[Chrome Extension 填單助手實戰](https://falo-taiwan.github.io/class/a01/class3/cases/chrome_extension/)
* 🔗 **ETL 實務**：[教材開發革命：從 PDF 到互動網站](https://falo-taiwan.github.io/class/a01/class3/cases/ipas-etl/)
* 🔗 **GAS 自動化**：[GAS 開發架構與部署實戰手冊](https://falo-taiwan.github.io/gas-demo/#gas-v3-security-practice)

---

## 7 大教學主軸與關聯資源

### 1. 多人共用多帳號下的 AI 協作模式
* **核心觀念**：記憶留在個人帳號，開發純開發。解耦多人共用環境，避免對話干擾與代碼洩漏。

### 2. Gemini_Canvas 常見困擾與解法
* **核心觀念**：突破 Canvas Token 限制，預防程式碼截斷與上下文漂移。
* 🔗 **課堂教材**：[LINE 防封鎖訊息變體生成器 (class03_computer_use.html)](class03_computer_use.html)

### 3. Antigravity / Codex 開發神器對比
* **核心觀念**：單點 API 邏輯撰寫 vs 全專案重構代理推理引擎。
* 🔗 **課堂教材**：[AI開發工具對比分析表 (agent_tools.html)](agent_tools.html)
* 🔗 **課堂教材**：[NotebookLM 升級與 Antigravity 實戰 (class03_notebooklm_upgrade.html)](class03_notebooklm_upgrade.html)
* 🔗 **進階手冊**：[Antigravity 實戰協作教學手冊](https://falo-taiwan.github.io/antigravity/)

### 4. md / html 雙軌文檔策略
* **核心觀念**：AI 讀 Markdown 節省 Token 召回記憶；人看 HTML 享受精美排版與微互動。以 logs 記錄決策脈絡。
* 🔗 **課堂教材**：[AI 協同討論與即時知識工程化實務 (class03_collaboration.html)](class03_collaboration.html)
* 🔗 **課堂教材**：[Chrome Gemini 解鎖與 FALO 六層模型案例 (class03_gemini_chrome.html)](class03_gemini_chrome.html)
* 🔗 **進階手冊**：[教材開發革命：從 PDF 到互動網站 (https://falo-taiwan.github.io/class/a01/class3/cases/ipas-etl/)](https://falo-taiwan.github.io/class/a01/class3/cases/ipas-etl/)

### 5. html based / rwd / 本機 & github prompt 部署
* **核心觀念**：利用 Prompt 輸出單一 RWD HTML 文件，雙擊本機啟動，一鍵發佈至 GitHub Pages。
* 🔗 **課堂教材**：[人與 AI 共創共生觀念教材 (class03_coexistence.html)](class03_coexistence.html)
* 🔗 **課堂教材**：[WWDC 26 Deep Research 實戰 Prompt 示範 (class03_wwdc_prompt.html)](class03_wwdc_prompt.html)
* 🔗 **進階手冊**：[Chrome Extension 填單助手實戰 (https://falo-taiwan.github.io/class/a01/class3/cases/chrome_extension/)](https://falo-taiwan.github.io/class/a01/class3/cases/chrome_extension/)

### 6. 地端 Server 架設與跨域對接
* **核心觀念**：本地 Python HTTP/API Server 架設，配合 ngrok 穿透內網，打通地雲數據傳輸。
* 🔗 **課堂教材**：[Vibe Coding 實戰武器庫與地雲工具 (class03_tools.html)](class03_tools.html)
* 🔗 **課堂教材**：[OCR 辨識策略與 HITL 人機協作 (class03_ocr_hitl.html)](class03_ocr_hitl.html)

### 7. Google Apps Script (GAS) 雲端自動化
* **核心觀念**：容器型與獨立型腳本開發，安全密鑰管理，避開 CORS OPTIONS 預檢限制。
* 🔗 **課堂教材**：[從 ERP 外掛到 AI Agent 演進與自動化治理 (class03_rpa_to_agent.html)](class03_rpa_to_agent.html)
* 🔗 **進階手冊**：[GAS 開發架構與部署實戰手冊](https://falo-taiwan.github.io/gas-demo/#gas-v3-security-practice)

---

## 輔助專區
* 🔗 **學員課前提問解答 (QA)**：[class03_qas.html](class03_qas.html)
