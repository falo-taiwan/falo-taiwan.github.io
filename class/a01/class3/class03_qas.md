# 銀河 ERP 內訓課程：Class 03 學員提問彙整 (class03_qas.md)
...
```
[Gemini Canvas] 產出 CSS/設計概念/排版 -> [人工過濾與抽離] -> [Codex/Antigravity] 模組化與架構化 -> 最終嚴謹代碼
```

1. **視覺設計期（Gemini Canvas / Figma）**：
   * 用 Gemini Canvas 來生成美觀的網頁佈局、色彩配對（使用 CSS Variables）、動畫效果與 RWD 樣式。
2. **邏輯工程化（Codex / Antigravity）**：
...
3. **串接與安全檢查**：
   * 最後由 Codex / Antigravity 負責資料驗證（Validation）、錯誤處理（Error Handling）與 API 的整合串接。
...
## Q12. 我們開發 APP 對於天心真的好維護嗎? 上平台驗證與付費？手機規格、不同 OS 的受限程度有多高?

### 💡 核心答覆
針對天心 ERP 這種企業內部 / 協同工作為主的系統，**非常不推薦開發「原生 App（Native App）」**，因為這會帶來極高的維護與平台上架負擔。
